...
# 其他在是非題當中如果出現9的代表有漏答
# v25_1~v25_5, v24_1~v24_13, v27_1~v27_14, v15_1~v15_11
# 迴圈的數值改成4:7, if後面改成c(4:7, 9)
...
rm(allNo, allNo_sum, del, emptyCol, miss, pickVar, year)
...
#------------居住地------------
# 發現居住地忘記處理遺漏值了
miss <- which(data_2016$resi >= 98)　　　# 手動修改要檢查的欄位
length(miss)
...
# 用SPSS轉換好了每年的居住地選項，匯入進R之後再merge起來

resi_16 <- read.csv("my documents/111-1 傳播導向的大數據分析/期末專案/問卷資料分析/resident place 2016.csv",
                    header = TRUE)
data_2016 <- merge(data_2016, resi_16, by.x = "no", by.y = "ID_16", all.x = TRUE)
data_2016 <- cbind(data_2016[, 1:2], data_2016[, 60:62], data_2016[, 3:58])
data_2016 <- data_2016[, -6]
colnames(data_2016)[3] <- "resi"
rm(resi_16)



resi_17 <- read.csv("my documents/111-1 傳播導向的大數據分析/期末專案/問卷資料分析/resident place 2017.csv",
                    header = TRUE)
resi_17 <- resi_17[, -2]
colnames(resi_17)[2] = "resi"
head(resi_17)
data_2017 <- merge(data_2017, resi_17, by.x = "no", by.y = "ID_17", all.x = TRUE)
head(data_2017)
data_2017 <- cbind(data_2017[, 1:2], data_2017[, 59:61], data_2017[, 4:58])
head(data_2017)
rm(resi_17)


resi_18 <- read.csv("my documents/111-1 傳播導向的大數據分析/期末專案/問卷資料分析/resident place 2018.csv",
                    header = TRUE)
head(resi_18)
#resi_18 <- resi_18[, -2]
colnames(resi_18)[2] = "resi"
head(resi_18)
data_2018 <- merge(data_2018, resi_18, by.x = "no", by.y = "ID_18", all.x = TRUE)
head(data_2018)
data_2018 <- cbind(data_2018[, 1:2], data_2018[, 59:61], data_2018[, 4:58])
head(data_2018)
rm(resi_18)


resi_19 <- read.csv("my documents/111-1 傳播導向的大數據分析/期末專案/問卷資料分析/resident place 2019.csv",                    
                    header = TRUE)
head(resi_19)
resi_19 <- resi_19[, -2]
colnames(resi_19)[2] = "resi"
head(resi_19)
data_2019 <- merge(data_2019, resi_19, by.x = "no", by.y = "ID_19", all.x = TRUE)
head(data_2019)
data_2019 <- cbind(data_2019[, 1:2], data_2019[, 59:61], data_2019[, 4:58])
head(data_2019)
rm(resi_19)



resi_20 <- read.csv("my documents/111-1 傳播導向的大數據分析/期末專案/問卷資料分析/resident place 2020.csv",                    
                    header = TRUE)
head(resi_20)
identical(data_2020$resi, as.numeric(resi_20$v02_20))
resi_20 <- resi_20[, -2]
head(resi_20)
data_2020 <- merge(data_2020, resi_20, by.x = "no", by.y = "ID_20", all.x = TRUE)
names(data_2020)
data_2020 <- cbind(data_2020[, 1:2], data_2020[, 59:61], data_2020[, 4:58])
colnames(data_2020)[3] = "resi"
head(data_2020)
rm(resi_20)
...
#------------族群------------
data_2019[which(data_2019$fami == 6), "fami"] = 90
data_2020[which(data_2020$fami == 6), "fami"] = 90


#------------行業------------
data_2019[which(data_2019$occu == 25), "occu"] = 90
data_2020[which(data_2020$occu == 25), "occu"] = 90
...
#------------ID------------
# 把year和no轉成character後用paste拼起來
ID <- paste(data_2016[, "year"], data_2016[, "no"], sep = "")
data_2016 = cbind(ID, data_2016)

ID <- paste(data_2017[, "year"], data_2017[, "no"], sep = "")
data_2017 = cbind(ID, data_2017)

ID <- paste(data_2018[, "year"], data_2018[, "no"], sep = "")
data_2018 = cbind(ID, data_2018)

ID <- paste(data_2019[, "year"], data_2019[, "no"], sep = "")
data_2019 = cbind(ID, data_2019)

ID <- paste(data_2020[, "year"], data_2020[, "no"], sep = "")
data_2020 = cbind(ID, data_2020)
...
#------------整合各年資料集------------
identical(colnames(data_2016), colnames(data_2017))
identical(colnames(data_2016), colnames(data_2018))
identical(colnames(data_2016), colnames(data_2019))
identical(colnames(data_2016), colnames(data_2020))

# 上面都是True才能跑下面的
data_5y <- rbind(data_2016, data_2017, data_2018, data_2019, data_2020)


# 把一些變數改成factor
data_5y$year <- factor(data_5y$year)

data_5y$resi <- factor(data_5y$resi)
levels(data_5y$resi) = c("基隆市","新北市","台北市","桃園市","新竹縣",
                         "新竹市","苗栗縣","台中市","彰化縣","南投縣",
                         "雲林縣","嘉義縣","嘉義市","台南市","高雄市",
                         "屏東縣","宜蘭縣","花蓮縣","台東縣","澎湖縣",
                         "金門縣","連江縣")

data_5y$region <- factor(data_5y$region)
levels(data_5y$region) = c("北部","中部","南部","東部","福建省")

data_5y$city <- factor(data_5y$city)
levels(data_5y$city) = c("直轄", "縣市")

data_5y$age <- factor(data_5y$age)
levels(data_5y$age) = c("15-19","20-24","25-29","30-34","35-39","40-44",
                        "45-49","50-54","55-59","60-64","65以上")

data_5y$edu <- factor(data_5y$edu)
levels(data_5y$edu)
levels(data_5y$edu) = c("小學及以下","初／國中","高中／職","專科大學","研究所及以上")

data_5y$fami <- factor(data_5y$fami)
levels(data_5y$fami)
levels(data_5y$fami) = c("閩南","客家","外省","原住民","新住民","其他")

data_5y$occu <- factor(data_5y$occu)
levels(data_5y$occu)

data_5y$inco <- factor(data_5y$inco)
levels(data_5y$inco)
levels(data_5y$inco) = c("0","0-10k","10-20k","20-30k","30-40k","40-50k",
                         "50-60k","60-70k", ">70k")

data_5y$sex <- factor(data_5y$sex)
levels(data_5y$sex)
levels(data_5y$sex) = c("男性", "女性")
